...
if($_GET['q']){
	...
	for ($y=1;$y<=100;$y++){
		...
		<div onclick="new Ajax.Updater('<?php echo $_GET['q'];?>', '/scale/mc.ajax?q=<?php echo $_GET['q'];?>&a=<?php echo $y;?>', {asynchronous:true,method:'get'}); return false;" class="scale"><?php echo $marker;?></div>
		<?php
	}
...
foreach ($aQuestions as $question) {
	...
	for ($y=1;$y<=100;$y++){
		$marker = ($y==$_GET['a'])?'<b style="color:red;">X</b>':(($y==50)?'|':'-');?>
		<div onclick="new Ajax.Updater('<?php echo $i;?>', '/scale/mc.ajax?q=<?php echo $i;?>&a=<?php echo $y;?>', {asynchronous:true,method:'get'}); return false;" class="scale"><?php echo $marker;?></div>
		<?php
	}
	echo '<div class ="scale">| 100%</div>';
...
?>
<form action = "/scale/setmc.php" method="POST">
